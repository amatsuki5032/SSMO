...
| `Netcode/RelayManager.cs` | RelayManager | Unity Relay接続（NAT越え・JoinCode・localhost直結フォールバック） |
...
- [x] **M3** (Week 23-28): 4v4 対戦モード（マッチメイキング・マップ・AI）
...
### INBOX タグルール
- タグなし → BACKLOG の「待機中」にタスクとして転記
- `[ideas]` タグ付き → BACKLOG の「## アイデア」セクションに転記（タスク化しない）
- アイデアセクションが存在しない場合は BACKLOG 末尾に `## アイデア` を作成して転記

### タスク完了時のアーカイブ
- 各タスク完了時、BACKLOGの完了セクションに移動したら、対応する指示書も即アーカイブする
- 移動先: instructions/m{X}-{Y}.md → docs/archive/m{X}/m{X}-{Y}.md
- アーカイブディレクトリがなければ mkdir -p で作成
