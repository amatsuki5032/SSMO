...
### ガード
- [x] LShift/U/O（L1）押しっぱなしでガード状態になる ✅M2-2c確認済（ステート遷移ログ確認）
- [ ] 正面からの攻撃を完全カット（0ダメージ）
- [ ] 通常ガード時にノックバックが発生する
- [x] ガード移動（LShift/U/O+WASD）ができる ✅M2-2c確認済（方向修正後）
- [ ] 無双乱舞もガードできる
- [ ] F5: 相手をガード状態に強制トグル
- [ ] めくり（背面攻撃）でガードを崩せる
- [ ] F1: 相手を正面2mに移動（正面ガードテスト）
- [ ] F6: 相手を背面2mに移動（めくりテスト）
...
- [ ] L2（Hキー）でブレイクチャージ（BC）が発動する
- [ ] ダッシュ中L2でダッシュブレイクチャージ（DBC）が発動する
- [ ] 空中でL2で空中ブレイクチャージ（JBC）が発動する
