...
### 接続
- [ ] Host起動が正常に完了する
- [ ] Clone側でClient接続が成功する
- [ ] 接続後にプレイヤーキャラ（箱人間）がスポーンする
- [ ] NetworkStatsHUD（左上）にRTT/PacketLossが表示される

### 移動同期
- [ ] WASD移動が両プレイヤーで正しく同期される
- [ ] 相手プレイヤーの動きがスムーズに見える（補間）
...
- [ ] N1→N2→N3→△ で C4（吹き飛ばし）が発動する
- [ ] 最終段（N4）からはチャージ派生不可

### ダッシュ攻撃
- [ ] 1.5秒以上移動後に□でダッシュ攻撃が発動する
...
### ガード
- [ ] Shift（L1）押しっぱなしでガード状態になる
- [ ] 正面からの攻撃を完全カット（0ダメージ）
- [ ] 通常ガード時にノックバックが発生する
- [ ] ガード移動（Shift+WASD）ができる
- [ ] 無双乱舞もガードできる
- [ ] F6: 相手をガード状態に強制トグル
- [ ] めくり（背面攻撃）でガードを崩せる
...
### 仙箪強化リング (EnhancementRing) ※M4-4b以降
- [ ] 仙箪7個でリング回転開始
- [ ] R1（Eキー）でスロット発動
- [ ] 連撃強化スロットでコンボ段数増加
- [ ] 死亡時に全強化リセット
